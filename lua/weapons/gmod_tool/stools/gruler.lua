...
if CLIENT then
	...
	language.Add( "tool.gruler.name", "Measurer Tool" )
	language.Add( "tool.gruler.desc", "A tool used for measuring purposes." )
	language.Add( "tool.gruler.left_1", "Set the Point 1" )
	language.Add( "tool.gruler.right_1", "Set the Point 2" )
	language.Add( "tool.gruler.reload", "Clear selection." )

end

-- Because gmod sucks.
local function GetClientValue(convar)
    local c = "gruler_" .. convar
...
local function SetClientData(convar, data)
    LocalPlayer():ConCommand("gruler_" .. convar .. " " .. data )
end
